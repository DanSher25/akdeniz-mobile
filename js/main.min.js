...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateUI() {
    // districts__item checked state (radio/checkbox labels)
  ...
  function openModal(html) {
    ...
    if (window.Fancybox?.bind) {
      window.Fancybox.bind(modalContent, "[data-fancybox]", {});
    }
  }
  ...
  function openSheet(html, trigger) {
    ...
    if (window.Fancybox?.bind) {
      window.Fancybox.bind(sheetContent, "[data-fancybox]", {});
    }
  }
  ...
  // Change events for filters -> recalc UI
  ...
  // Global click delegation (не ломаем fancybox)
  document.addEventListener("click", (e) => {
    // Fancybox must receive native click
    if (e.target.closest("[data-fancybox]")) return;

    // Close buttons: modal/sheet
    const closeBtn = e.target.closest(".bs__close");
    if (closeBtn) {
      e.preventDefault();

      if (isModalOpen() && closeBtn.closest("#filtersModal")) {
        closeModal();
        return;
      }
      ...
      // если bs__close оказался в модалке без id-обёртки
    ...
    // Favorites
    const fav = e.target.closest(".header__favorite, .apartments-aside__favorite, .card__favorite, .gallery-modal__favorite");
    if (fav) {
      e.preventDefault();
  ...
  // Districts dropdown (multi-checkbox) - отдельно, чтобы не мешать остальному
  ...
  (function initGalleryHeaderFooterScroll() {
    ...
    const observer = new MutationObserver(() => {
      const modal = document.querySelector("#filtersModal.is-open");
      if (!modal) return;
