...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const OVERLAY_DEPTH_KEY = "__overlayDepth__";

  const modal = qs("#filtersModal");
  const modalContent = modal ? qs(".modal__content", modal) : null;

  const sheet = qs("#textSheet");
  const sheetPanel = sheet ? qs(".bs__panel", sheet) : null;
  const sheetContent = sheet ? qs(".bs__content", sheet) : null;
  const sheetOverlay = sheet ? qs(".bs__overlay", sheet) : null;

  const lockScroll = () => (document.body.style.overflow = "hidden");
  const unlockScroll = () => (document.body.style.overflow = "");

  const normalizeDigitsAndSpaces = (value) =>
    String(value || "")
      .replace(/[^\d]/g, "")
      .replace(/\B(?=(\d{3})+(?!\d))/g, " ");

  const getVal = (name) => qs(`[name="${CSS.escape(name)}"]`)?.value ?? "";
  const setValAll = (name, value) => qsa(`[name="${CSS.escape(name)}"]`).forEach((i) => (i.value = value));
  ...
  function ensureBaseHistoryState() {
    ...
    if (typeof st[OVERLAY_DEPTH_KEY] !== "number") {
      history.replaceState({ ...st, [OVERLAY_DEPTH_KEY]: 0 }, "");
    }
  }

  function getHistoryDepth() {
    return history.state && typeof history.state[OVERLAY_DEPTH_KEY] === "number" ? history.state[OVERLAY_DEPTH_KEY] : 0;
  }

  function setScrollLockByStack() {
    if (overlayStack.length > 0) lockScroll();
    else unlockScroll();
  }

  function closeOverlayUI(type) {
    if (type === "sheet") {
      if (!sheet || !sheetContent || !sheetPanel) return;
      sheet.classList.remove("is-open", "is-full");
      sheetPanel.style.transform = "";
      sheetContent.innerHTML = "";
      sheet.removeAttribute("data-no-swipe-close");
    }
  ...
  function syncToDepth(targetDepth) {
    while (overlayStack.length > targetDepth) {
      const top = overlayStack.pop();
      closeOverlayUI(top);
    }
    setScrollLockByStack();
  }
  ...
  function pushOverlay(type) {
    ...
    if (overlayStack.length > depth) syncToDepth(depth);

    overlayStack.push(type);

    const st = history.state || {};
    history.pushState({ ...st, [OVERLAY_DEPTH_KEY]: overlayStack.length }, "");
    setScrollLockByStack();
  }
  ...
  function requestClose(type) {
    ...
    const steps = overlayStack.length - idx;
  ...
  window.addEventListener("popstate", () => {
    syncToDepth(getHistoryDepth());
  });

  function updateUI() {
    qsa(".districts__item").forEach((el) => {
      const input = qs("input", el);
      if (!input) return;
      el.classList.toggle("is-checked", !!input.checked);
    });

    qsa(".filter__price-field").forEach((field) => {
      const input = qs(".filter__price-input", field);
      if (!input) return;
      field.classList.toggle("active", input.value.trim() !== "");
    });

    const priceType = getVal("price_type") || "total";
    qsa("[data-switch]").forEach((switchEl) => {
      const items = qsa(".filter-switch__item", switchEl);
      const color = qs(".filter-switch__color", switchEl);
      const idx = Math.max(
        0,
        items.findIndex((b) => b.dataset.value === priceType)
      );
    ...
    const hasPropertyType = !!qs('input[name="property_type"]:checked');
    const hasRegion = !!qs('input[name="region"]:checked');
    const hasRooms = qsa('input[name="rooms[]"]:checked').length > 0;
    const hasDistrict = qsa('input[name="district[]"]:checked').length > 0;

    const priceFrom = getVal("price_from").trim();
    const priceTop = getVal("price_top").trim();
    const hasPrice = !!(priceFrom || priceTop);
    const hasNonDefaultPriceType = priceType !== "total";

    let count = 0;
    if (hasPropertyType) count += 1;
    if (hasRegion) count += 1;
    if (hasRooms) count += 1;
    if (hasDistrict) count += 1;
    if (hasPrice) count += 1;
    if (hasNonDefaultPriceType) count += 1;
  ...
  function buildQuery() {
    ...
    const propertyType = qs('input[name="property_type"]:checked')?.value || "";
    ...
    const priceFrom = getVal("price_from").replace(/\s/g, "");
    const priceTop = getVal("price_top").replace(/\s/g, "");
    if (priceFrom) params.set("price_from", priceFrom);
    if (priceTop) params.set("price_top", priceTop);

    qsa('input[name="rooms[]"]:checked').forEach((i) => params.append("rooms[]", i.value));
    qsa('input[name="district[]"]:checked').forEach((i) => params.append("district[]", i.value));
  ...
  function openModal(html) {
    ...
    if (window.Fancybox?.bind) {
      Fancybox.bind(modalContent.querySelectorAll("[data-fancybox]"), {});
    }
  }
  ...
  function openSheet(html, trigger) {
    ...
    sheet.toggleAttribute("data-no-swipe-close", trigger?.hasAttribute("data-no-swipe-close"));
    ...
    if (window.Fancybox?.bind) {
      Fancybox.bind(sheetContent.querySelectorAll("[data-fancybox]"), {});
    }
  }

  function closeAllDropdowns(except = null) {
    qsa(".dropdown-list").forEach((dd) => {
      if (dd === except) return;
      if (dd.classList.contains("districts-dropdown")) return;

      dd.classList.remove("active");
      const wrap = qs(".dropdown-list__wrapper", dd);
  ...
  if (window.Swiper) {
    qsa(".card__slider").forEach((sliderEl) => {
  ...
  (function initMobileMenu() {
    ...
    const menu = qs(".header__mobile-menu");
    const overlay = qs(".header__overlay");
    const close = qs(".header__mobile-close");
    ...
    qsa("a", menu).forEach((a) => a.addEventListener("click", hide));
  ...
  document.addEventListener("input", (e) => {
    ...
    if (!input || !input.matches(".filter__price-input")) return;
  ...
  document.addEventListener("change", (e) => {
    const el = e.target;
    if (!el || !el.matches("input[name]")) return;

    if (el.name === "price_from" || el.name === "price_top") {
      el.value = normalizeDigitsAndSpaces(el.value);
    }
  ...
  (function initSheetDrag() {
    if (!sheetPanel || !sheet) return;

    let startY = 0;
    let currentY = 0;
    let dragging = false;

    const swipeDisabled = () => sheet.hasAttribute("data-no-swipe-close");
    ...
    sheetPanel.addEventListener("touchmove", (e) => {
      ...
      if (currentY > 0) sheetPanel.style.transform = `translateY(${currentY}px)`;
    });
    ...
    sheetPanel.addEventListener("touchend", () => {
      ...
      if (!swipeDisabled() && currentY > 80) requestClose("sheet");
  ...
  if (sheetOverlay) sheetOverlay.addEventListener("click", () => requestClose("sheet"));

  document.addEventListener("click", (e) => {
    if (e.target.closest("[data-fancybox]")) return;

    const closeBtn = e.target.closest(".bs__close");
    if (closeBtn) {
      e.preventDefault();

      if (closeBtn.closest("#textSheet")) requestClose("sheet");
      else if (closeBtn.closest("#filtersModal")) requestClose("modal");

      return;
    }

    if (e.target.closest("[data-modal-close]") && isModalOpen()) {
      e.preventDefault();
      requestClose("modal");
      return;
    }
    ...
    const switchBtn = e.target.closest("[data-switch] .filter-switch__item");
    if (switchBtn) {
      e.preventDefault();
      const value = switchBtn.dataset.value;
      if (value) setValAll("price_type", value);
      updateUI();
      return;
    }

    const resetAll = e.target.closest(".filter__all-reset");
    if (resetAll) {
      e.preventDefault();

      qsa('input[name="property_type"]').forEach((i) => (i.checked = false));
      qsa('input[name="region"]').forEach((i) => (i.checked = false));
      qsa('input[name="rooms[]"]').forEach((i) => (i.checked = false));
      qsa('input[name="district[]"]').forEach((i) => (i.checked = false));

      setValAll("price_type", "total");
      setValAll("price_from", "");
      setValAll("price_top", "");
    ...
    const showBtn = e.target.closest(".filter__show-btn");
    if (showBtn) {
      e.preventDefault();
      submitFilters();
      return;
    ...
    const accTrigger = e.target.closest(".accordion__trigger");
    if (accTrigger) {
      e.preventDefault();

      const accordion = accTrigger.closest(".accordion");
      if (!accordion) return;

      const wrapper = qs(".accordion__wrapper", accordion);
      const content = wrapper ? qs(".accordion__content", wrapper) : null;
    ...
    const bsTrigger = e.target.closest(".bs-trigger");
    if (bsTrigger) {
      e.preventDefault();

      const targetId = bsTrigger.dataset.bsTarget;
      if (!targetId) return;

      const contentEl = document.getElementById(targetId);
      if (!contentEl) return;

      const openType = bsTrigger.dataset.bsOpen;
      ...
      openSheet(contentEl.innerHTML, bsTrigger);
      return;
    }

    const ddItem = e.target.closest(".dropdown-list__dropdown-item");
    if (ddItem) {
      const dd = ddItem.closest(".dropdown-list");
      ...
      const textEl = qs(".dropdown-list__text", dd);
      const inputEl = qs(".dropdown-list__input", dd);
      const wrap = qs(".dropdown-list__wrapper", dd);

      const value = ddItem.dataset.type || ddItem.dataset.city || ddItem.textContent.trim();
    ...
    const dd = e.target.closest(".dropdown-list");
    if (dd && !dd.classList.contains("districts-dropdown")) {
      toggleDropdown(dd);
      return;
    }
  ...
  (function initDistrictsDropdown() {
    const dropdown = qs(".districts-dropdown");
    if (!dropdown) return;

    const wrapper = qs(".dropdown-list__wrapper", dropdown);
    const textEl = qs(".dropdown-list__text", dropdown);
    const hiddenInput = qs(".districts-input", dropdown);
    const items = qsa(".district-item", dropdown);

    dropdown.addEventListener("click", (e) => {
      if (e.target.closest(".district-item")) return;
  ...
  (function initSticky() {
    const sentinel = qs(".sticky-sentinel");
    const sticky = qs(".filter__sticky");
  ...
  (function initGalleryHeaderFooterScroll() {
    let lastScrollTop = 0;
    let hidden = false;

    function init(container) {
      const header = container.querySelector(".gallery-modal__header");
      const footer = container.querySelector(".gallery-modal__btns");
      ...
      container.addEventListener("scroll", () => {
        ...
        if (st > lastScrollTop && st > 50 && !hidden) {
          header.classList.add("is-hidden");
          footer.classList.add("is-hidden");
          hidden = true;
        }

        if (st < lastScrollTop && hidden) {
    ...
    const observer = new MutationObserver(() => {
      const m = document.querySelector("#filtersModal.is-open");
      if (!m) return;
